...
export function docsPlugin(options: PluginOptions = {}): Plugin {
  ...
  let samples: Samples = { components: [], blocks: [] };
  let basePath = "/";
  ...
  async function initialize(config: any) {
    ...
    try {
      // Scan content and sample directories
      ...
      const totalSamples = samples.components.length + samples.blocks.length;

      console.log(
        `[vite-plugin-docs] Indexed ${contents.length} content files and ${totalSamples} samples`,
      );
    } catch (error) {
  ...
  return {
    ...
    async load(id) {
      if (id === VIRTUAL_CONTENTS_ID) {
        return generateContentsModule(contents, samplesDir, true);
      }
      if (id === VIRTUAL_SAMPLES_ID) {
        return generateSamplesModule(samples, samplesDir);
      }
      return null;
    },
