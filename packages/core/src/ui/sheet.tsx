...
function SheetOverlay({
  ...
  return (
    <BaseSheet.Backdrop
      ...
      className={cn(
        "fixed inset-0 bg-black/50 transition-all duration-200 [&[data-ending-style]]:opacity-0 [&[data-starting-style]]:opacity-0 z-50",
        className,
      )}
      {...props}
    />
  ...
  return (
    <SheetPortal>
      ...
      <BaseSheet.Popup
        ...
        className={cn(
          "bg-popover text-popover-foreground fixed z-50 flex max-h-[calc(100vh-2rem)] flex-col gap-4 shadow-lg outline-hidden transition ease-in-out data-closed:duration-300 data-open:duration-500",
          side === "right" &&
        ...
        style={{ borderRadius: "var(--radius-boxes)", ...style }}
        {...props}
      >
        {children}
        <SheetClose
          className="ring-offset-popover focus:ring-ring text-muted-foreground absolute top-4 right-4 opacity-50 transition-opacity hover:opacity-100 focus:ring-[3px] focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4"
          style={{ borderRadius: "var(--radius-selectors)" }}
        >
